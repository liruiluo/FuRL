...
class VLMAgent:
    ...
    def __init__(self,
        ...
        self.rng, actor_key, critic_key, W_key = jax.random.split(self.rng, 4)
    ...
    def get_reward(self, image_embedding):
        reward = optax.cosine_similarity(image_embedding, self.text_embedding)
        return reward
    ...
    def critic_train_step(self, batch: Batch, key: Any, alpha: float,
                          actor_state: train_state.TrainState,
                          critic_state: train_state.TrainState,
        ...
        # reward shaping
        rewards = batch.rewards + self.rho * batch.vlm_rewards
        (_, log_info), grads = grad_fn(critic_state.params,
                                       ...
                                       batch.discounts)
        extra_log_info = {"q_max": log_info["q"].max()}
        grads = jax.tree_util.tree_map(functools.partial(jnp.mean, axis=0), grads)
        log_info = jax.tree_util.tree_map(functools.partial(jnp.mean, axis=0), log_info)
