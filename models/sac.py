...
class SACAgent:
    ...
    def __init__(self,
        ...
        # Dummy inputs
        self.dummy_obs = jnp.ones([1, obs_dim], dtype=jnp.float32)
        ...
        actor_params = self.actor.init(actor_key, actor_key, self.dummy_obs)["params"]
        self.actor_state = train_state.TrainState.create(
            apply_fn=self.actor.apply, params=actor_params, tx=actor_tx)

        # Initialize the Critic
        self.critic = DoubleCritic(hidden_dims=hidden_dims)
        critic_params = self.critic.init(critic_key, self.dummy_obs, dummy_act)["params"]
